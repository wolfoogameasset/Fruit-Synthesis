...
public class Config : SingletonGetMono<Config>
{
    ...
    public static Action<int> OnRandomNextItem;
    ...
    internal int nextValue = 0;
    internal int value = 0;

    //edit: Add New next value
    internal void CreateDefute()
    {
        //value = m_GetStartIndex[isRangeResult ? getResult(m_StartRange, 100) : m_Result];
        ...
        else
        {
            ...
            {
                value =  m_GetStartIndex[isRangeResult ? getResult(m_StartRange, 100) : m_Result];
                nextValue = m_GetStartIndex[isRangeResult ? getResult(m_StartRange, 100) : m_Result];
                OnRandomNextItem?.Invoke(nextValue);
                isFirstSpawn = false;
            }
            else
            {
